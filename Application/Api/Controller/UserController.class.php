...
class UserController extends MobileController
{
	...
	//实名认证处理
	public function upauthrz(/*$phone,*/$real_name,$cardzm,$cardfm,$area,$rztype){
	...
	/*	$uinfo = M("user")->where(array('phone'=>$phone))->find();
		if(!empty($uinfo)){
	       $this->ajaxReturn(['code'=>0,'info'=>L('该手机号已被他人绑定')]);
		}*/
	...
	public function txcoin(){
	   ...
	   $clist = M("coin")->where(array('txstatus'=>1,'default_on'=>1))->field("id,name,title")->select();
	    $minfo = M("user_coin")->where(array('userid'=>$uid))->find();
	...
	public function paycoin(){
	    if($_POST){
	        ...
	        $coinname = trim(I('post.coinname'));
	        $info = M("coin")->where(array('id'=>$cid))->find();
	        $czline = $info['czline'];
	        $czaddress =$info['czaddress'];
