...
// }
		parent::_initialize();	$allow_action=array("index","edit","status","admin","adminEdit","adminBind","adminStatus","updateRules","log","logEdit","logStatus","qianbao","qianbaoEdit","qianbaoStatus","coin","coinEdit","coinFreeze","coinLog","setpwd","amountlog","loginadmin","billdel","sendnotice","upsendnotice","noticelist","noticedel","authrz","upanthrz","setmoney","online","onlinelist","sendonline","uponline","setagent","agent","cancelagent","buy_zhengchang","buy_shu","buy_ying","buy_off","buy_on","adminlog","adminlogedit","getlineinfo","uptxt");
...
//        $info['rztypestr'] = L($info['rztypestr']);
	...
	//提交聊天内容
	public function uptxt($txt = null,$uid=null){

	    if (checkstr($txt) ) {
			$this->ajaxReturn(['code'=>0,'info'=>L('您输入的信息有误')]);
		}
		$uinfo = M("online")->where(array('uid'=>$uid))->field("uid,username")->find();
		if($uid <= 0){
		    $this->ajaxReturn(['code'=>0,'info'=>L('您输入的信息有误')]);
		}
		...
		if($result){
			 M("online")->where(array('uid'=>$uid))->save(array('state'=>1));
	...
	public function online($name=NULL, $field=NULL, $status=NULL){
	   $where['state'] = 0;
$distinct = true; // 使用 distinct 方法
$count = M('online')->where($where)->count('DISTINCT username', $distinct);
$Page = new \Think\Page($count, 50);
$show = $Page->show();

$list = M('online')->field('id,uid,username,count(username)as nor,addtime')->where($where)->group('username')->order('addtime desc')->limit($Page->firstRow . ',' . $Page->listRows)->select();
...
$this->display();
		...
		if (empty($_POST)) {
		    ...
		    if($id <= 0 || $id == null){
               ...
               /* $add['money']  =0.0000;
                ...
                //记录日志
                if($add['password']){
                    insert_admin_log(UID, '修改用户密码', '修改用户'.$re['id'].'密码');
                }
		        if($re){
		    ...
		    //编辑会员   
		       ...
		       if ($_POST['password']) {
		        ...
		        if($result){
                    ...
                    if ($_POST['paypassword'] == $user['paypassword']) {
                        $this->error("新交易密码与旧交易密码一致");exit();
                    }
	...
	public function adminEdit(){
		if (empty($_POST)) {
		 ...
		 if (UID!="1"){
			...
			if ($_POST['id']) {
                //记录操作日志
                insert_admin_log(UID,'编辑管理员');
				$rs = M('Admin')->save($input);
			} else {
                //记录操作日志
                insert_admin_log(UID,'新增管理员');
				$_POST['addtime'] = time();
    ...
    public function adminBind(){
        if (empty($_POST)) {
            if (empty($_GET['id'])) {
                ...
                M('Admin')->where(array(array('id' => trim($_GET['id']))))->save(array('secret' => $secret));
                $admin=M('Admin')->where(array('id' => trim($_GET['id'])))->find();
                $this->data = M('Admin')->where(array('id' => trim($_GET['id'])))->find();
                $domain = $_SERVER['HTTP_HOST']."";
                $qrCodeUrl = $google->getQRCodeGoogleUrl($admin['username'], $admin['secret'],$domain);
               

                $url = $qrCodeUrl;

                $drpath = './Public/Static/coinimgs/';
                $imgma = $admin['username'] . '.png';
            ...
            $input = I('post.');
            $admin=M('Admin')->where(array('id' => $_POST['id']))->find();
            $google=new GoogleAuthenticator();
            $checkResult = $google->verifyCode($admin['secret'], $input['verify'], 2);
            if (!$checkResult){
                $this->error('谷歌验证码错误');
            }
            ...
            if ($_POST['id']) {
                $drpath = './Public/Static/coinimgs/';
                $imgma = $admin['username'] . '.png';
                $filename = './Public/Static/coinimgs/' . $imgma;
                if(file_exists($filename)){
                    unlink($filename);
                }
                //记录操作日志
                insert_admin_log(UID,'启用谷歌验证码');
                $rs=  M('Admin')->where(array('id' => $_POST['id']))->save(array('ck' => 1));
                if ($rs) {
                    $this->success('绑定成功！', U('User/admin'));
                } else {
                    $this->error('编辑失败！');
                }
            }
    ...
    //管理员操作日志
    public function adminlog($name = NULL, $field = NULL, $status = NULL)
    {
        $where = array();
        if ($field && $name) {
            if ($field == 'username') {
                $where['admin_id'] = M('Admin')->where(array('username' => $name))->getField('id');
            } else {
                $where[$field] = $name;
            }
        }
    ...
    //编辑登陆记录
    public function adminlogedit($id = NULL){
        if (empty($_POST)) {
            if (empty($id)) {
                $this->data = null;
            } else {
                $this->data = M('AdminLog')->where(array('id' => trim($id)))->find();
            }
	...
	public function buy_ying($id){
        insert_admin_log(UID, '指定必赢', '指定用户 '.$id. ' 必赢');
	    $uid = $id;
	    if($uid <= 0 || $uid == ''){
	        $this->error("参数得要参数");
	    }
	...
	public function buy_shu($id){
        insert_admin_log(UID, '指定必输', '指定用户 '.$id. ' 必输');
	    $uid = $id;
	    if($uid <= 0 || $uid == ''){
	        $this->error("参数得要参数");
	    }
	...
	public function buy_zhengchang($id){
        insert_admin_log(UID, '正常输赢', '让用户 '.$id. ' 正常输赢');
	    $uid = $id;
	    if($uid <= 0 || $uid == ''){
	        $this->error("参数得要参数");
	    }
