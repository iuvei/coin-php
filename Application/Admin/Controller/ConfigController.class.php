	...
	public function coinEdit($id = NULL)
	{
		if (empty($_POST)) {
		   ...
		   if($bbbb){
			...
			if ($_POST['id']) {
			     insert_admin_log(UID, '修改内容', '编辑币种配置'.json_encode($_POST). ' 内容');
...
*/
			...
			if ($rs) {
			     insert_admin_log(UID, '添加内容', '添加币种配置'.json_encode($_POST). ' 内容');
	...
	public function coinStatus()
	{
		...
		switch (strtolower($method)) {
			...
			case 'delt':
				...
				insert_admin_log(UID, '删除内容', '删除币种配置'.json_encode($rs). ' 内容');
