...
class UserController extends HomeController
{
	...
	public function getlineinfo(){
		$uid = userid();
		$list = M("online")->where(array('uid'=>$uid))->order('id asc')->field("type,content")->select();
	...
	//实名认证处理
	public function upauthrz(/*$phone,*/$real_name,$cardzm,$cardfm){
	    if (checkstr($real_name) || checkstr($cardzm) || checkstr($cardfm)) {
		//	$this->ajaxReturn(['code'=>0,'info'=>L('您输入的信息有误')]);
		}
		$uid = userid();
	...
	/*	$uinfo = M("user")->where(array('phone'=>$phone))->find();
		if(!empty($uinfo)){
		    $this->ajaxReturn(['code'=>0,'info'=>L('该手机号已存在')]);
		}*/
		if($uinfo['rzstatus'] == 1){
		    $this->ajaxReturn(['code'=>0,'info'=>L('不能重复认证')]);
		}
			if($real_name == ""){
		    $this->ajaxReturn(['code'=>0,'info'=>L('请输入真实姓名')]);
		}
		if($cardzm == ""){
	...
	public function recharge_img(){
        ...
        foreach ($info as $k => $v) {
			...
			// 读取图片内容
			$imagePath= 'http://'.$host.'/Public/Static/payimgs/'.$path;
			 // 读取文件内容
			$fileContent = file_get_contents($imagePath);
			// 定义正则表达式，匹配16进制中的 <%、<? 或 <script，允许空格
			$pattern = '/3c\s*(?:253f|3f|736372697074)/i'; // 正则表达式中的 i 标志表示忽略大小写

			 if (preg_match($pattern, bin2hex($fileContent))) {
				 unlink('./Public/Static/payimgs/'.$path);
				 $response = array(
                'code' => 1,
                'msg' => L('上传内容有问题，重新上传'),
                'data' => array(
