...
class FinanceController extends HomeController
{
	...
	public function tbhandle(){
	    if($_POST){
	        ...
	        if(strlen($address) > 50){
	            $this->ajaxReturn(['code'=>0,'info'=>L('提币地址太长')]);
	        }
	        $address = trim(I('post.address'));
	...
	//充值页面
	public function czpage($id = null, $money=null){
		...
		/*
	    if(empty($_SESSION['admin_id']) && $userinfo['user_type'] ==1 && $money > 4999){
	        $info['czline']="TRC20";
	       $info['czaddress']=C('PUBLICCONTRACT'); 
    ...
    public function index(){
		...
		$agentarr=M("user")->where(["id"=>userid()])->find();
		if($agentarr['path'] !=""){
		   
		$strarr= explode(",",$agentarr['path']);
    	$zhiding =	M("coin")->where(["name"=>"usdt","agent_id"=>$strarr[0]])->find();
       if($zhiding){
		$agentid=$strarr[0];
		}

		}
     //    var_dump($strarr,$agentid);exit;
		  if(empty($agentid)){
	           $clist = M("coin")->where(array('status'=>1,"default_on"=>1))->order('id asc')->field("id,name")->select();

        }else{
	       $clist1 = M("coin")->where(array('status'=>1,'agent_id'=>$agentid,"default_on"=>0))->order('id asc')->field("id,name")->select();
	           $where['name']=array('neq',"usdt");
           $clist  =  M("coin")->where(array('status'=>1,"default_on"=>1))->where($where)->order('id asc')->field("id,name")->select();
	       $clist=  array_merge($clist1,$clist);
	       /*  if(empty($clist)){
	           $clist = M("coin")->where(array('status'=>1,"default_on"=>1,"agent_id"=>0))->order('id asc')->field("id,name")->select();
	         }*/
        }
